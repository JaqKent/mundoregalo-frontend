  ...
  return (
    <>
      { transition((style, item) => (item && (
        <animated.div
          style={{
            ...style, position: 'fixed', zIndex: 999, top: 0, left: 0,
          }}
        ...
        >
          <nav className={styles.container}>
            ...
            { isLoggedIn && LOGGED_USER_OPTIONS.map((nav) => (
              <NavigationItem
                {...nav}
                key={nav.id}
              />
            ))}
            {xs && <NavigationItem icon={faTimes} name="Cerrar menú" onClick={toggleNavbar} />}
          </nav>
        </animated.div>
      )))}
      {backdropTransition((style, item) => item && (
      <animated.div style={style}>
        <div className={styles.backdrop} onClick={toggleNavbar} role="none" />
      </animated.div>
      ))}
    </>
  );
